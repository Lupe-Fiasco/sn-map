    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const map = validateMapConfig(row.map_config);
                // ai coding：row.snapshot 已由安全 RPC 在数据库内白名单重建，浏览器从不接收原始快照。
                if (active) setData({ loading: false, error: "", row, bounds: map.bounds, map, types });
            } catch (error) {
