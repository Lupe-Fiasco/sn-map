    ...
    return <>
        ...
        {data.loading ? <main className="share-message" role="status">正在加载公开快照…</main> : data.error ? <main className="share-message error" role="alert"><h2>无法打开公开地图</h2><p>{data.error}</p></main> : (
            <main className="public-main">
                ...
                <aside className="info-panel"><section className="place-card"><p className="section-label">快照地点</p><h2>地点列表</h2>{details && <div className="public-details" aria-live="polite"><h3>{details.name}</h3><dl>{details.type && <div><dt>类型</dt><dd>{details.type}</dd></div>}{details.coordinates && <div><dt>经纬度</dt><dd>{details.coordinates}</dd></div>}{details.description && <div><dt>备注</dt><dd>{details.description}</dd></div>}</dl></div>}<ul className="place-list public-place-list">{data.row.snapshot.features.map((feature) => <li key={feature.id}><button type="button" onClick={() => setSelectedId(feature.id)} aria-pressed={selectedId === feature.id}><span className="type-swatch" style={{ background: data.types.find((type) => type.id === feature.properties.type)?.color || "#65736f" }} /><span className="place-name">{feature.properties.name}</span><span className="place-type">{feature.geometry.type === "Polygon" ? "区域" : "点"}</span></button></li>)}</ul>{!data.row.snapshot.features.length && <p className="empty-state">该快照没有地点。</p>}<p className="file-status">最后发布：{new Date(data.row.published_at).toLocaleString("zh-CN")}</p></section></aside>
            </main>
        )}
